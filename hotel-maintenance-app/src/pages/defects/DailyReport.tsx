...
import { getTodayDefects, getDefects } from '../../data/mockDefects';
...
export default function DailyReport() {
    ...
    const [todayData, setTodayData] = useState<{ reported: Defect[]; resolved: Defect[] }>({ reported: [], resolved: [] });
    const [allDefects, setAllDefects] = useState<Defect[]>([]);

    useEffect(() => {
        setTodayData(getTodayDefects());
        setAllDefects(getDefects());
    }, []);
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                        <div className="bg-white rounded-xl border border-gray-200 p-4">
                            <div className="flex items-center gap-2 mb-2">
                                ...
                                <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Reported Today</span>
                            </div>
                            <p className="text-3xl font-bold text-gray-900">{reportedTodayCount}</p>
                        </div>
                        ...
                        <div className="bg-white rounded-xl border border-gray-200 p-4">
                            ...
                            <p className="text-3xl font-bold text-gray-900">{resolvedTodayCount}</p>
                        </div>

                        <div className="bg-white rounded-xl border border-gray-200 p-4">
                            <div className="flex items-center gap-2 mb-2">
                                <AlertTriangle className="w-4 h-4 text-orange-500" />
                                <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Still Open</span>
                            </div>
                                ...
                                {todayData.reported.map(d => (
                                    <div key={d.id} className="p-4 flex items-center gap-4">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <p className="text-xs text-gray-500">{d.location} · {d.reportedBy}</p>
                                        </div>
                            ...
                            <div className="divide-y divide-gray-50">
                                {todayData.resolved.map(d => (
                                    <div key={d.id} className="p-4">
                                        ...
                                        {d.resolutionNotes && (
                                            <p className="text-xs text-gray-500 ml-7 bg-gray-50 rounded-lg p-2 mt-1">{d.resolutionNotes}</p>
                                        )}
                                    </div>
                                ))}
                            </div>
                        ...
                        <div className="p-6">
                            <div className="grid grid-cols-3 gap-4 text-center">
                                <div>
                                    ...
                                    <p className="text-xs text-gray-500 mt-1">PM Tasks Scheduled</p>
                                </div>
                                <div>
                            ...
                            <p className="text-xs text-gray-400 text-center mt-4">PM data will be pulled from the live schedule once Supabase is connected.</p>
                        </div>
                    </div>
