...
export default function WorkerManagement() {
    ...
    const workerStats = workers.map(w => {
        ...
        return {
            ...w,
            avatar: (w.full_name ?? 'U').split(' ').map(n => n[0]).join('').substring(0, 2).toUpperCase(),
            activeTasks: hash % 4,
            completed,
            onTime,
            overdue,
    ...
    const completionRate = (w: typeof workerStats[0]) =>
        w.completed > 0 ? Math.round((w.onTime / w.completed) * 100) : 0;
    ...
    return (
        <div className="min-h-screen bg-gray-50 flex font-sans">
            ...
            <main className="flex-1 p-8 overflow-y-auto">
                ...
                {loading ? (
                    <div className="flex items-center justify-center h-40 text-gray-400">Loading workers...</div>
                ) : (
                    <>
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
                            <div className="bg-white rounded-xl border border-gray-200 p-4">
                                <p className="text-sm text-gray-500 mb-1">Total Workers</p>
                                <p className="text-2xl font-bold text-gray-900">{workerStats.length}</p>
                            </div>
                            <div className="bg-white rounded-xl border border-gray-200 p-4">
                                <p className="text-sm text-gray-500 mb-1">Available Now</p>
                                <p className="text-2xl font-bold text-green-600">{workerStats.filter(w => w.available).length}</p>
                            </div>
                            <div className="bg-white rounded-xl border border-gray-200 p-4">
                                <p className="text-sm text-gray-500 mb-1">Active Tasks</p>
                                <p className="text-2xl font-bold text-blue-600">{workerStats.reduce((s, w) => s + w.activeTasks, 0)}</p>
                            </div>
                            <div className="bg-white rounded-xl border border-gray-200 p-4">
                                <p className="text-sm text-gray-500 mb-1">Avg On-Time Rate</p>
                                <p className="text-2xl font-bold text-indigo-600">
                                    {workerStats.length > 0 ? Math.round(workerStats.reduce((s, w) => s + completionRate(w), 0) / workerStats.length) : 0}%
                                </p>
                            </div>
                        </div>
                            ...
                            {workerStats.map(w => (
                                <div key={w.id} className="bg-white rounded-2xl shadow-sm border border-gray-200 p-5 hover:shadow-md transition-shadow">
                                        ...
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <div className="flex items-center gap-2">
                                                    <h3 className="font-bold text-gray-900">{w.full_name ?? 'Unnamed'}</h3>
                                                    {w.employment_type === 'staff' && (
                                                        <img src={staffLogo} alt="Staff" className="w-4 h-4 object-contain" title="Internal Staff" />
                                                    )}
                                                </div>
                                                <p className="text-xs text-gray-500">{w.sub_category ?? 'General Maintenance'}</p>
                                            </div>
                                        </div>
                                    ...
                                    <div className="grid grid-cols-3 gap-3 mb-4">
                                        {[
                                            { val: w.activeTasks, label: 'Active', color: 'text-blue-600' },
                                            { val: w.completed, label: 'Done', color: 'text-gray-900' },
                                            { val: `${completionRate(w)}%`, label: 'On-Time', color: completionRate(w) >= 90 ? 'text-green-600' : completionRate(w) >= 70 ? 'text-amber-600' : 'text-red-600' },
                                        ].map(({ val, label, color }) => (
                                    ...
                                    <div className="mb-4">
                                        ...
                                        <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                                            <div className={cn('h-full rounded-full',
                                                completionRate(w) >= 90 ? 'bg-green-500' : completionRate(w) >= 70 ? 'bg-amber-500' : 'bg-red-500')}
                                                style={{ width: `${completionRate(w)}%` }} />
                                        </div>
                                    </div>
